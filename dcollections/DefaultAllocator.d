 ...
 * elements.  This makes allocating individual elements more efficient because
 * the GC isn't used for allocating every element, only every chunk of
 * elements.
 *
 * The only requirement is that the size of V is >= size of a pointer.  This
 * is because the data V contains is used as a pointer when freeing the
 * element.
 *
 * If an entire chunk of elements is freed, that chunk is then returned to the
 * GC.
 */
struct ChunkAllocator(V, uint elemsPerChunk)
{
    ...
    struct chunk
    {
        ...
        /**
         * The previous chunk in the chain.  Required for O(1) removal from
         * the chain.
         */
        chunk *prev;
    ...
    /**
     * The fresh chunk.  This is only used if no elements are available in the
     * used chain.
     */
    chunk *fresh;
    
    /**
     * The next element in the fresh chunk.  Because we don't worry about the
     * free list in the fresh chunk, we need to keep track of the next fresh
     * element to use.
     */
    uint nextFresh;
    ...
    V* allocate()
    {
        ...
        //
        // no used elements are available, allocate out of the fresh elements
        //
        if(fresh is null)
        {
            fresh = new chunk;
    ...
    void free(V* v)
    {
        ...
        {
            static assert(false, "don't support non-tango");
        }

        if(cur !is fresh && cur.numFree == 0)
        {
            //
            // move cur to the front of the used list, it has free nodes to be
            // used.
            //
            if(cur !is used)
            {
